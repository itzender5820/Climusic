...
public:
    ...
    static bool parse(const std::string& path, Settings& s) {
        ...
        // BUGFIX (audit C5): a mid-read I/O error (disk error, file
        // truncated/deleted concurrently) used to be silently accepted —
        // src would just be whatever was read so far, parsed as if it
        // were the complete, valid file. badbit specifically indicates a
        // real I/O failure (as opposed to eofbit, which istreambuf_iterator
        // triggers normally on a clean, complete read).
    ...
    static bool write_back(const std::string& path, const Settings& s) {
        // BUGFIX (audit D5): read-modify-write with no locking at all —
        // two instances of the app saving settings around the same time
        // could interleave and corrupt the file. flock() is POSIX
        // (Linux/Android/macOS — this project's actual targets); held for
        // the whole read-modify-write sequence via a separate fd so the
        // ifstream/ofstream below can keep using normal iostream calls.
        int lock_fd = open(path.c_str(), O_RDWR);
        if (lock_fd >= 0) flock(lock_fd, LOCK_EX);
        auto unlock = [&] { if (lock_fd >= 0) { flock(lock_fd, LOCK_UN); close(lock_fd); } };

        std::ifstream in(path);
        if (!in.is_open()) { unlock(); return false; }
        ...
        if (!out.is_open()) { unlock(); return false; }
