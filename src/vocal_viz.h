...
public:
    ...
    std::array<float, N_SLOTS> compute() {
        ...
        for (int s = 0; s < N_SLOTS; ++s) {
            ...
            // than intended. Not memory-unsafe (the buffer is always
            // fully initialized), but a real correctness bug: instead of
            // comparing the intended same-time-window shifted by lag, it
            // silently correlated against unrelated stale audio, most
            // noticeably degrading accuracy on the outer (high-lag)
            // columns. Pulling the window back by the positive lag amount
            // keeps every ri at or behind the write head.
            int end_pullback = std::max(0, lag);
