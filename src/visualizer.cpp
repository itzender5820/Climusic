...
VisualizerFrame Visualizer::render(VizStyle style, int bands) {
    ...
    // on those same members (called on terminal resize) — a real
    // reallocation-during-read/write race. Holding the lock for the whole
    // render call (both styles, matching the discipline SCOPE already had)
    // closes that window; render() is called once per frame from the main
    // thread, so this isn't a contention hotspot worth optimizing around.
    std::lock_guard lk(mtx_);
